...
const PageContentArea: React.FC = () => {
  return (
    <Page>
      ...
      <Text>
        ...
        layout (ContentLayout) and embedding the content bar (ContentWithBar).
        If you use ContentLayout, ContentStyle will be applied automatically. If
        you use ContentWithBar, ContentLayout (and because of this also
        ContentStyle) will be applied.
      </Text>
    ...
    appliedStyles?: StylesArray<typeof ContentStyleStyles>;
      ...
      <Text>
        This means that you are able to pass an array of predefined styles to
        the component which should be activated. The predefined styles are the
        following:
      </Text>
      <Code language={"language"}>
    ...
    SET_SPACING_COLOR: "SET_SPACING_COLOR",
      ...
      <Text>
        The framework also exports templates. These are just predefined arrays
        of styles. These are the following:
      </Text>
      <BulletList
        bulletType={"bullet"}
        items={[
...
/>`}
      ...
      <Text>
        Using the ContentLayout component you are able to specify a content
        layout for your content area component. Furthermore, the ContentLayout
        component uses the ContentStyle component. To use ContentStyle with
        ContentLayout you have to pass an object containing the appliedStyles
        array to ContentLayout.
      </Text>
    ...
    layoutBehaviour?: LayoutBehaviour; // Option to specify the layout which is one option of the following enum.
      ...
      <Text>The LayoutBehaviour enum is defined like this:</Text>
      <Code language={"typescript"}>
...
export const ExampleComponent = () => {
  return (
    <ContentLayout layoutBehaviour={LayoutBehaviour.GRID} contentStyle={appliedStyles: ContentStyleTemplates.CONTENT_CELLS}>
      ...
      <Text>The ContentCell component will be explained later on.</Text>
      <SubSubTitle>The ContentWithBar component</SubSubTitle>
      <Text>
        This HOC can be used for implementing a content area component
        containing a Content Bar. To embed the Content Bar you just have to pass
        a wrapper array containing your components to the ContentWithBar
        component using it`s &quot;contentWrappers&quot; property. The
        ContentWithBar component will render the Content Bar and underneath it
        will render your content area UI which is the child of this component.
      </Text>
      <Text>
        To define the tabs for the Content Bar and the corresponding components
        which will be shown inside the content area, you have to define a
        wrapper object array similarly to the array for the navigation bar. This
        time you have to create an array of instances of the class
        BasicContentbarWrapper or CustomContentbarWrapper.
        BasicContentbarWrapper renders the simplest and mainly used content tab.
        CustomContentbarWrapper allows you to define a custom component which
        will be rendered inside the content bar. The array is then passed to the
        Content component using the contentWrappers property.
      </Text>
      <Text>
        Furthermore, the framework will render buttons for sliding to the left
        and right inside the content bar. While the navigation bar isn&#39;t
      ...
      <Text>
        ...
        functionality of these components will be added by default. The
        following code snippet shows the properties of the Content component.
      </Text>
      <Code title={"PageInterface Content"} language={"typescript"}>
    ...
    onClickRightSlideButton?: () => any;// Using this property you can pass a function that will be triggerd if the slide right Button is clicked.
      ...
      <Code
        title={"Constructor BasicContentbarWrapper"}
        language={"typescript"}
      >
        {`constructor({
    ...
    closable?: boolean, // Property to define if a closing icon will be rendered.
      ...
      <Code
        title={"Constructor CustomContentbarWrapper"}
        language={"typescript"}
      >
        {`constructor({
  id: string, // Identifier of the tab element
  ...
  contentAreaElement: React.ReactElement // Defines which custom component should be rendered when selecting a content bar tab.
      ...
      <Code
        title={"Example Content Component Implementation"}
        language={"typescript"}
      >
  ...
  return (
    ...
    />
      ...
      <Text>
        <strong>Hint:</strong> see further integration with different use cases
        and React State Management hooks in the Example-Project and
        TemplateProject
      </Text>
      ...
      <Text>
        ...
        component has to be embedded inside ContentLayout (or ContentWithBar as
        it uses ContentLayout) with your desired content area layout. The
        following code snippet shows it&#39;s properties.
      </Text>
      <Code
        title={"PageInterface ContentCell and CellPaddings ENUM"}
        language={"typescript"}
      >
...
export interface Props {
  ...
  paddings: CellPaddings; // Definition of the element's padding.
      ...
      <Text>
        <strong>Important hint:</strong> This component has to be embedded
        inside ContentLayout (or ContentWithBar) with the layout set to GRID.
      </Text>
      <Code
        title={"Example implementation with the ContentCell components"}
        language={"typescript"}
      >
...
export const ContentbarExampleWithText = (props: Props) => {
  return (
    <>
      ...
      <Text>The resulting content area looks like this:</Text>

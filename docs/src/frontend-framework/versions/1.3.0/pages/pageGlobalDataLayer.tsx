...
export const LayoutAndContextExampleComponent = applyTranslation(FirstExampleComponentUnprocessed);`}
    ...
    initialLang?: string; // The key of the language which will be initially used if no language cookie is set yet. For example this option can be used to define the used language when the language selection is hidden.
      ...
      <Text>
        ...
        separated it&#39;s possible to mix authentication providers and views
        with each other. You are also able to just develop a new authentication
        provider without developing a new authentication view.
      </Text>
      <Text>
        The framework provides two authentication providers and two
        authentication views. However, you can implement custom authentication
        providers (for example for authentication with Microsoft Azure) and own
        authentication views.
      </Text>
      <SubSubTitle>DummyAuthenticationProvider</SubSubTitle>
      <Text>
        <em>DummyAuthenticationProvider</em> is the default authentication
        provider (which will get used if nothing is specified by you) and allows
        every combination of email and password. This authentication provider is
        intended to be used while developing when there is no backend
        authentication service yet. This enables you to show a realistic
        workflow to your client. You are able to pass additional values and
        functions as an object of key-value pairs (for example{" "}
        <code>{`{ getUserGroups: () => [] }`}</code>) to{" "}
        <em>DummyAuthenticationProvider</em> by using the property{" "}
        <em>additionalContextValues</em>. These values and functions will be
        shared using the auth context. To do this you have to include the{" "}
        <em>DummyAuthenticationProvider</em> above the <em>GlobalDataLayer</em>{" "}
        on your own (as shown in the beginning of this documentation). The{" "}
        <em>GlobalDataLayer</em> component detects that the authentication
        provider has been changed and will skip the default process. The
        properties of the <em>DummyAuthenticationProvider</em> are defined as
        follows:
      </Text>
      <Code
        title={"PageInterface AWSAuthenticationProvider"}
        language={"typescript"}
      >
    ...
    additionalContextValues?: { [key: string]: any }; // Optional object to share values and function via the context of the DummyAuthenticationProvider
      ...
      <Text>
        ...
        Cognito. To use this authentication provider you have to configure
        Amplify first. To do this you have to define a configuration function
        and pass it to the AWSAuthenticationProvider . You do this by including
        the AWSAuthenticationProvider above the GlobalDatalayer (as shown in the
    ...
    legalGroups?: string[]; // Optional parameter to define the legal groups
      ...
      <Code
        title={"Properties AWSAuthenticationProvider"}
        language={"typescript"}
      >
...
function App  {
  ...
  <AWSAuthenticationProvider configureAmplify={() => { Amplify.configure(authConfig); }}>
    ...
    <GlobalDataLayer .../>
      ...
      <Text>
        In the process of implementing an authentication provider you have to
        keep some aspects in mind:
      </Text>
      <BulletList
        bulletType="number"
        items={[
          ...
          "Finally you have to wrap the PageGlobalDataLayer and all other components inside your authentication provider (for example in the render method of your App.tsx) (as shown in the beginning of this documentation).",
        ]}
      />
      ...
      <Text>
        Hint: The process of implementing a custom authentication view is
        content of the video tutorials.
      </Text>
      <Text>
        The following code snippet shows an example for the implementation of an
        authentication view. It should subscribe to the AuthContext react
        context in order to be able to trigger methods like the login method.
      </Text>
      <Code language={"typescript"}>

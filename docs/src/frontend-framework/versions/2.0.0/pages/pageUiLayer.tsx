  ...
  return (
    ...
    disableLogin?: boolean; // Option to disable the login and logout.
      ...
      <Text>
        ...
        navigation bar. However, you are able to implement custom navigation
        tabs. All navigation tabs have to have the properties of the following
        interface:
      </Text>
    ...
    active?: boolean;
      ...
      <Text>
        ...
        component. It&#39;s properties are just the navbarTabProps interface.
        The second navigation tab component is the PrivilegedNavbarTab
        component. This component allows acces if the user belongs to the passed
        array of groups. The user&#39;s group is retrieved using the
        authentication provider. PrivilegedNavbarTab has the properties of
        navbarTabprops and extends them by the following interface:
      </Text>
      <Code language={"typescript"}>
        {`interface Props {
    permittedGroups: string[]; // Mandatory array of groups which should have access to the content associated with this navigation tab.
      ...
      <Text>
        To implement custom navigation tabs you should follow the structure of
...
`}
      ...
      <Text>
        ...
        PrivilegedNavbarTab has all the properties of the interface
        NavbarTabProps and additionally all properties of the interface Props.
        This is because the navbarTab type is defined as follows:
      </Text>
 ...
 >;`}
      ...
      <Text>
        ...
        If you implement your component in this way there is nothing more you
        have to do regarding <em>frameworkInjectedOptions</em>. However, if you
        want to implement your component without using SimpleNavbarTab you
        should do research on the use of <em>frameworkInjectedOptions</em> in
        ...
        section. To implement such a factory you can examine the ones provided
        by the Framework.
      </Text>
      <SubSubTitle>Using navigation tab components</SubSubTitle>
      <Text>
        You&#39;ve learned how to use existing navigation tabs and how to
        develop custom ones. But how do you pass navigation tabs to the
        framework and connect them to a specific component for the content area
        which should be rendered if the tab is selected?
      </Text>
      <Text>
        For this purpose the framework provides the wrapper class
        <em>BasicContentWrapper</em>. It wrapps a factory for the element which
        is rendered in the navigation bar (the navigation tab) and the component
        which is rendered in the content area. By wrapping these two elements
        inside an instance of BasicContentWrapper the framework is able to
        assign each navigation tab to a content area element. In order to
        specify all your navigation tabs and the corresponding content area
        elements you have to{" "}
        <strong>
          create an array of instances of this BasicContentWrapper
        </strong>
    ...
    protected _component: React.ComponentType                           // The element which will be rendered inside the content area.
      ...
      <Text>
        Furthermore, the framework provides the <em>Group</em> wrapper class.
        This class allows you to specify groups of navigation tabs with a
        specified label. To create a group you have to pass the corresponding
        array of BasicContentWrapper instances to the Group instance as a
        parameter. The Group class has the following parameters:
      </Text>
    ...
    private _contentWrappers: TabAndContentWrapper[] // Array which contains all BasicContentWrapper instances which are part of the group.
      ...
      <Text>
        The following code snippet shows an example for defining navigation
        tabs, a group and the corresponding content:
      </Text>
...
const views = [
    ...
    new Group(
      ...
      <Text>
        ...
        factories: One for SimpleNavbarTab, one for PrivilegedNavbarTab. These
        are simpleNavbarTabFactory and privilegedNavbarTabFactory. You have to
        pass them the properties which will be passed to the navigation tab
        component (for example SimpleNavbarTab) itself.
      </Text>
      ...
      <Text>
        ...
        Because of this the property &quot;fill&quot;, which represents the
        color has to be set to the value &quot;current&quot;. The width and
        height of the icons should be set to 24px. The following code snippet
        shows an example svg element.
      </Text>
      <Text>
        <strong>Hint 1:</strong> We recommend to use the UI/UX Tool Figma for
        Prototyping. Figma offers the functionality to export icons. The
        framework supports the exported figma icons after the setting of the
    ...
    <path id="info_FILL0_wght300_GRAD0_opsz24" d="M11.25,16.75h1.5V11h-1.5ZM12,9.3a.757.757,0,0,0,.575-.238.82.82,0,0,0,.225-.587.792.792,0,0,0-.225-.563.816.816,0,0,0-1.15,0,.792.792,0,0,0-.225.563.82.82,0,0,0,.225.587A.757.757,0,0,0,12,9.3Zm0,12.2a9.263,9.263,0,0,1-3.712-.75A9.432,9.432,0,0,1,3.25,15.712a9.563,9.563,0,0,1,0-7.425A9.435,9.435,0,0,1,8.288,3.25a9.563,9.563,0,0,1,7.425,0A9.437,9.437,0,0,1,20.75,8.287a9.563,9.563,0,0,1,0,7.425,9.435,9.435,0,0,1-5.037,5.038A9.27,9.27,0,0,1,12,21.5ZM12,20a7.721,7.721,0,0,0,5.675-2.325A7.721,7.721,0,0,0,20,12a7.721,7.721,0,0,0-2.325-5.675A7.721,7.721,0,0,0,12,4,7.721,7.721,0,0,0,6.325,6.325,7.721,7.721,0,0,0,4,12a7.721,7.721,0,0,0,2.325,5.675A7.721,7.721,0,0,0,12,20ZM12,12Z" transform="translate(-0.5 0.5)" fill="current"/>
      ...
      <SubSubTitle>
        SettingsMenuOptions: How to configure the settings menu?
      </SubSubTitle>
      <Text>
    ...
    hideColorThemeToggler?: boolean; // Option to hide the toggle component for changing the theme (light and dark mode).
      ...
      <SubSubTitle>
        UserMenuOptions: How to configure the user menu?
      </SubSubTitle>
      <Code language={"typescript"}>
    ...
    headerElements?: ReactElement[]; // User can add their own elements to the header
      ...
      <Text>
        If the user wants to add their own elements in the header, they can
        either use their own React elements or fall back on the
      ...
      <SubSubTitle>
        AuthOptions: How to configure the authentication view?
      </SubSubTitle>
      <Text>
        These options (property of the UILayer component) allow you to configure
...
export interface AuthenticationViewProps {
    ...
    hideLegalDocuments?: boolean;
      ...
      <SubSubTitle>
        NavbarOptions: How to configure the navigation bar?
      </SubSubTitle>
      <Code language={"typescript"}>

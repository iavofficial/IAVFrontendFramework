  ...
  (props, ref) => {
    ...
    const activeLang = useSelector(
      (state: DefaultRootState) =>
        state[MandatoryModuleNames.Internationalizer].activeLang,
    );
    ...
    if (!props.menuOptions?.hideLanguageSelection) {
      ...
      Object.keys(intModule.translationResources).forEach((key) => {
        if (key !== intModule.fallbackLang) {
          ...
          languageOptions.push({
            ...
            command: () => {
              intModule.selectActiveLang(key.replaceAll("_", "-"));
            },
          });
          if (active) {
      ...
      languageOptions.push({
        label:
          intModule.translationResources[intModule.fallbackLang].translation
            .option_name,
        icon: !notFallbackLang ? "pi pi-check" : "",
        command: () => intModule.selectActiveLang(intModule.fallbackLang),
      });
...
function containsLanguage(lang: string, resources: LangResources) {

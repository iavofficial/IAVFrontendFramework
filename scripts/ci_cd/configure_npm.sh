...
curl -s -u$artifactory_user:$artifactory_apikey https://artifactory.iav.com/artifactory/api/npm/auth | awk '{printf "https://artifactory.iav.com/artifactory/api/npm/auth:%s\n", $0}' >> .npmrc
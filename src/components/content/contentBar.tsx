...
export const ContentBar = (props: PropsContentBar) => {
    ...
    }, [handleDivResize, lastElementIsVisible, navbarSettingsContext?.navbarCollapsed]);
    ...
    const handleSlideRightEvent = useCallback(() => {
        if (startRenderElements + amountOfRenderedTabElements < contentElements.length) {
            if (onClickRightSlideButton) {
                onClickRightSlideButton();
            }
            setStartRenderElements((startRenderElements) => startRenderElements + 1);
        }
    }, [amountOfRenderedTabElements, contentElements.length, onClickRightSlideButton, startRenderElements]);
